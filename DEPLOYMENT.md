...
- `NEXT_PUBLIC_YANDEX_MAPS_API_KEY` - API ключ для Яндекс Карт (см. инструкцию ниже)

### 2. Получить API ключ для Яндекс Карт

API ключ необходим для работы карты в футере сайта.

#### Шаг 1: Регистрация

1. Перейди на https://developer.tech.yandex.ru/
2. Войди в аккаунт Яндекс или зарегистрируйся

#### Шаг 2: Получение ключа

1. Перейди в "Мои сервисы" → "JavaScript API"
2. Нажми "Подключить API" → "JavaScript API и HTTP Геокодер"
3. Нажми "Получить ключ"
4. Укажи название проекта (например, "White Wolf Clinic")
5. **ВАЖНО:** В поле "Источники запросов" укажи свой домен (например, `yourdomain.com`)
6. Скопируй полученный ключ и добавь в `.env`:

```env
NEXT_PUBLIC_YANDEX_MAPS_API_KEY=твой-ключ-здесь
```

**Подробная инструкция:** см. `ENV_TEMPLATE.md`

### 3. Настроить домен в конфигах
...
### 5. Подключиться к серверу и установить Docker
...
### 7. Клонировать проект на сервер
...
```bash
# Проверь, установлен ли API ключ
docker compose exec whitewolf env | grep YANDEX

# Если переменная не установлена или пустая:
# 1. Добавь NEXT_PUBLIC_YANDEX_MAPS_API_KEY в .env файл на сервере
# 2. Перезапусти контейнер
docker compose up -d --build

# Проверь логи браузера (F12 → Console) на ошибки API
# Убедись, что домен добавлен в разрешенные источники в консоли Яндекс
```

**Частые причины:**

- ❌ Переменная `NEXT_PUBLIC_YANDEX_MAPS_API_KEY` не установлена в `.env`
- ❌ Домен не добавлен в разрешенные источники в консоли разработчика Яндекс
- ❌ Превышен лимит запросов (25,000/день на бесплатном тарифе)
- ❌ API ключ скопирован с пробелами или неполностью
